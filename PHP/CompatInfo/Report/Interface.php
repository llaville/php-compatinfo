...
class PHP_CompatInfo_Report_Interface extends PHP_CompatInfo_Report
{
    ...
    public function generate($report, $base, $verbose)
    {
        if ($verbose < 3) {
            ...
            foreach ($files as $filename) {
                foreach ($report[$filename]['interfaces'] as $extension => $data) {
                    foreach ($data as $key => $values) {
                        if (!isset($interfaces[$extension][$key])) {
                            $interfaces[$extension][$key] = $values;
                        } else {
                            $interfaces[$extension][$key]['uses']   
                                += $values['uses'];
                            $interfaces[$extension][$key]['sources'] = array_merge(
                                $interfaces[$extension][$key]['sources'], 
                                $values['sources']
                            );
                        }
                    }
    ...
    {
        echo str_repeat('-', $this->width) . PHP_EOL;
        echo 'A TOTAL OF ' . count($this->total) .' INTERFACE(S) WERE FOUND';
