...
class PHP_CompatInfo_Report_Namespace extends PHP_CompatInfo_Report
{
    ...
    public function generate($report, $base, $verbose)
    {
        if ($verbose < 3) {
            ...
            foreach ($files as $filename) {
                foreach ($report[$filename]['namespaces'] as $extension => $data) {
                    foreach ($data as $key => $values) {
                        if (!isset($namespaces[$extension][$key])) {
                            $namespaces[$extension][$key] = $values;
                        } else {
                            $namespaces[$extension][$key]['uses']
                                += $values['uses'];
                            $namespaces[$extension][$key]['sources'] = array_merge(
                                $namespaces[$extension][$key]['sources'],
                                $values['sources']
                            );
                        }
                    }
    ...
    {
        echo str_repeat('-', $this->width).PHP_EOL;
        echo 'A TOTAL OF ' . count($this->total) . ' NAMESPACE(S) WERE FOUND';
